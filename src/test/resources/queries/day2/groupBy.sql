...
-- for each job_id we should write different queries which does NOT make sense...
-- so we should use group by

-- group by --> group the data based on provided column first,
-- then implement aggregate function (avg here)
...
--more detailed: avg salary + number of employees + sum of salary + min and max salary for each dept
SELECT JOB_ID,AVG(SALARY),COUNT(*),SUM(SALARY),MIN(SALARY),MAX(SALARY)
FROM EMPLOYEES
GROUP BY JOB_ID;


--cannot use 'first_name' bc group by is used for grouping so each group has no first_name
...
--TASK: How many locations we have for each country US UK CA where location count is bigger than 2
